...
import { ChevronLeft, Clock, Info, Play, RefreshCcw } from "lucide-react";
...
export default function RoutineDetail() {
  ...
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center bg-[#f5f5f5]">
        <RefreshCcw className="h-10 w-10 text-[#ff0000] animate-spin stroke-[2.5px]" />
      </div>
    );
  }
  ...
  if (!routine) {
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-[#f5f5f5] p-6 text-center space-y-4">
        <h2 className="text-2xl font-black uppercase tracking-tight">Rutina no encontrada</h2>
        <Button className="border-[3px] border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]" onClick={() => navigate(-1)}>VOLVER</Button>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <header className="flex items-center p-6 pb-4">
        ...
        <h1 className="text-xl font-black uppercase tracking-tight ml-2 truncate">{routine.title?.toUpperCase()}</h1>
      </header>
      ...
      <div className="px-6 space-y-6">
        <motion.div variants={{ hidden: { opacity: 0, scale: 0.95 }, show: { opacity: 1, scale: 1 } }} className="bg-black text-white p-6 rounded-3xl relative overflow-hidden group shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] border-[3px] border-black">
          ...
          <div className="relative z-10 space-y-4">
            <h2 className="text-3xl font-black tracking-tight leading-none group-hover:text-[#ff0000] transition-colors">{routine.title}</h2>
            <div className="flex items-center gap-4 text-sm font-bold text-gray-300">
              <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" /> {routine.time}</span>
              <span className="flex items-center gap-1.5 text-[#ff0000]"><Info className="h-4 w-4" /> {routine.level}</span>
            </div>
          </div>
        </motion.div>

        <div>
          <h3 className="text-lg font-black tracking-tight mb-4 flex items-center gap-2">Lista de ejercicios ({exercises.length})</h3>
